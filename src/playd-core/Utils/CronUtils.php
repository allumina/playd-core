...
use Allumina\Playd\Core\Exceptions\CronException;

class CronUtils
{
    const YEARS_MIN = 1970;
    const YEARS_MAX = 2037;
    ...
    final public static function parse($specification = '* * * * * * *')
    {
        if(count($elements = preg_split('/\s+/', $specification)) < 5) // Split input liberal. Single or multiple Spaces, Tabs and Newlines are all allowed as separators.
            throw new CronException('Invalid specification.');
        ...
        $output = new CronUtils();
        $output->cronMinutes        = $output->cronInterpret($elements[0],                               0,                           59, array(),            'minutes');
        $output->cronHours        = $output->cronInterpret($elements[1],                               0,                           23, array(),            'hours');
        $output->cronDaysOfMonth    = $output->cronInterpret($elements[2],                            1,                           31, array(),            'daysOfMonth');
        $output->cronMonths        = $output->cronInterpret($elements[3],                            1,                           12, $arrMonths,        'months');
        $output->cronDaysOfWeek    = $output->cronInterpret($elements[4],                            0,                            6, $arrDaysOfWeek,    'daysOfWeek');

        $output->minutes = $output->cronCreateItems($output->_cronMinutes);
        $output->hours = $output->cronCreateItems($output->_cronHours);
        $output->daysOfMonth = $output->cronCreateItems($output->_cronDaysOfMonth);
        $output->months = $output->cronCreateItems($output->_cronMonths);
        $output->daysOfWeek = $output->cronCreateItems($output->_cronDaysOfWeek);

        if (isset($elements[5])) {
            $output->_cronYears = $output->cronInterpret($elements[5], self::YEARS_MIN, self::YEARS_MAX, array(), 'years');
            $output->_years = $output->cronCreateItems($output->_cronYears);
        }
    ...
    final public function match($time = FALSE)
    {
        ...
        // Minutes
        if(!array_key_exists($arrDT[0], $this->minutes)) return FALSE;
